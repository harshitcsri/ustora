...
<script src="https://oss.maxcdn.com/html5shiv/3.7.2/html5shiv.min.js"></script>
 ...
 <?php
    ...
    $product_title=$_GET['id'];
    $product_price=$_GET['price'];
    // Create connection
    

$sql = "SELECT product_title,product_price FROM product";
...
if($name==''||$email=='' ||$mobile=='' ||$address=='' ||$pin=='' ||$state==''|| $country=='')
{
    ...
    exit();
	 ...
	 $uid=$_SESSION['usr_id'];
      $insert_query="insert into test(name,email,mobile,address,pin,state,country,product_id,usercart,orderdate,price) values('$name','$email','$mobile','$address','$pin','$state','$country','$product_title','$uid',curdate(),$product_price)";
   
  $run_query=mysqli_query($con,$insert_query) or die ("Error while Inserting");
  if( $run_query)
  {
     echo "<script>alert('Product addred to Cart');location.href='cart.php'; </script>";
  }
   else{
